...
# Better error reporting (especially when bash prints confusing truncations like "...: command not found")
trap 'rc=$?; echo "" >&2; echo "✗ ERROR (exit ${rc})" >&2; echo "  File: ${BASH_SOURCE[0]}" >&2; echo "  Line: ${BASH_LINENO[0]}" >&2; echo "  Cmd : ${BASH_COMMAND}" >&2; exit ${rc}' ERR
...
download_file() {
    ...
    if [ "${USE_POWERSHELL_DOWNLOAD:-}" != "1" ] && command -v curl >/dev/null 2>&1; then
        ...
        if command -v powershell.exe >/dev/null 2>&1; then
            echo "Info: curl download failed (exit ${rc}); retrying with powershell.exe..." >&2
        else
            echo "✗ FAILED: curl download failed (exit ${rc})." >&2
            if [ -n "${err_text}" ]; then
                echo "curl error:" >&2
                echo "${err_text}" >&2
            fi
...
if ! docker --version >/dev/null 2>&1; then
    ...
    if docker run --rm -v "$(pwd):/work" kubesec/kubesec:v2 scan /work/rendered-dev.yaml > kubesec-results.json 2>/dev/null; then
        # Check if any workloads got scored
        if grep -q '"object": "Deployment\|"object": "StatefulSet' kubesec-results.json 2>/dev/null; then
        ...
        echo "FAILED: Kubesec scan failed"
    ...
    echo "Running Trivy..."
    echo "-----------------------------------------"
    if docker run --rm -v "$(pwd):/work" aquasec/trivy:latest config /work/rendered-dev.yaml --severity HIGH,CRITICAL; then
    ...
    echo "-----------------------------------------"
    if docker run --rm -v "$(pwd):/work" bridgecrew/checkov:latest -f /work/rendered-dev.yaml --framework kubernetes --compact --quiet; then
